...
4. **Workout vertical slice** — complete: start/resume/discard, timestamp-based
   warm-up/working timers, per-slot alternatives and defaults, completion, previous
   results, and history.
5. **Configuration and backup** — complete: plan/slot/exercise configuration,
   deactivation, increments, local exercise images, versioned export, validated
   confirmed import, and pre-import backup.
...
exercise names are unique. Imports are limited to 50 MiB for resized local images.
ADRs 0001–0008 contain the
architectural rationale.

## Later candidates, not commitments

Additional units/languages, synchronization, encrypted backups, repetitions,
